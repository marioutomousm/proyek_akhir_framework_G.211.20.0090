                ...
                <div class="card">
                    <div class="card-body">
                        <div class="col-lg-8 mt-3">
                            {{-- <form action="{{ url('/barangMasuk/store'. $barangMasuk->id) }}" method="POST"> --}}
                            <form action="{{ url('barangMasuks/'.$barangMasuk->id) }}" method="POST">
                                @method('PUT')
                                @csrf
                                <div class="form-group">
                                    <label>Nama Barang</label>
                                    <input type="text" name="nama_barang" class="form-control" value="{{ old('nama_barang', $barangMasuk->nama_barang) }}" autofocus>
                                ...
                                <div class="form-group">
                                    ...
                                    value="{{old('kode_barang',$barangMasuk->kode_barang)}}">
                                    ...
                                    <select name="toko_id" class="form-control">
                                        ...
                                        @foreach ($tokos as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('toko_id',$barangMasuk->toko_id) == $item->id ? 'selected' : null }}>
                                                {{ $item->nama_toko }}
                                            </option>
                                        @endforeach
                                    </select>
